// client/src/Components/EXPIRE/ExpiredReport.js
  ...
  const fetchReport = async (overrides = {}) => {
    ...
    try {
      ...
      const sorted = [...data].sort((a, b) => {
        const tA = toTimestamp(a.note_date);
        const tB = toTimestamp(b.note_date);
        if (tA !== tB) return tA - tB; // oldest first
        const noteA = a.note_no || "";
        const noteB = b.note_no || "";
        return String(noteA).localeCompare(String(noteB));
      });

      setRows(sorted);
    } catch (err) {
      console.error("Error loading expire report:", err);
      if (err.response?.status === 401) safeLogout();
      else setMessage("❌ Failed to load expire report.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="max-w-7xl mx-auto mt-8 px-4 md:px-6 pb-10">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-semibold text-gray-800">Expired Stock Movements</h1>
                <p className="text-sm text-gray-500 mt-1">
                  Track expired store inbound and outbound quantities with filters and printing.
                </p>
              </div>
            </div>
