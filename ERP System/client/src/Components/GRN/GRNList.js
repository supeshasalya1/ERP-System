// client/src/Components/GRN/GRNList.js
...
const GRNList = () => {
  ...
  const [printTarget, setPrintTarget] = useState(null); // null = full list, id = single GRN
  ...
  const fetchGrns = async () => {
    ...
    try {
      ...
      const res = await axios.get("/api/stocks/grn/filter", { headers, params });
      ...
      const prefetched = await Promise.all(
        data.slice(0, 10).map(async (g) => {
          try {
            const ir = await axios.get(`/api/stocks/grn/items/${g.grn_id}`, {
              headers,
            });
      ...
      setGrns(data);
      const primed = {};
      prefetched.forEach((g) => (primed[g.grn_id] = g.__items));
      ...
      if (err.response?.status === 401 || err.response?.status === 403)
        safeLogout();
      else setMessage("❌ Failed to load GRNs.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchItems = async (grnId) => {
    if (itemsByGrn[grnId]) return; // cached
    try {
      const res = await axios.get(`/api/stocks/grn/items/${grnId}`, { headers });
  ...
  const formatQty = (item) => {
    ...
    const pcs = toInt(item.pieces_received ?? item.quantity_received, 0);
  ...
  const ItemRow = ({ it }) => {
    ...
    const pcs = toInt(it.pieces_received ?? it.quantity_received, 0);
    const boxes = toInt(it.boxes_received, Math.floor(pcs / pack));
    const items = toInt(it.items_received, pcs % pack);
  ...
  return (
    <>
      <div className="no-print">
        ...
        {/* match AddProduct / Create GRN layout */}
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="bg-indigo-600 text-white px-5 py-3 rounded-xl hover:bg-indigo-700 text-lg"
              >
                + Add GRN
              </button>
            </div>
      ...
      <div className="print-area">
        <div style={{ fontFamily: "Arial, sans-serif", fontSize: 11 }}>
          <div style={{ textAlign: "center", marginBottom: 8 }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, marginTop: 6 }}>Goods Received Notes</div>
          </div>
          ...
          {printableGrns.map((g) => {
            ...
            const showPerNotePrinted = Boolean(printTarget); // single GRN prints only
            return (
