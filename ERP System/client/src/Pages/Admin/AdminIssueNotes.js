// client/src/Pages/Admin/AdminIssueNotes.jsx
...
const AdminIssueNotes = () => {
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      console.error("Error fetching suppliers (admin issue notes):", err);
  ...
  const fetchNotes = async () => {
    ...
    try {
      ...
      if (supplierId) params.supplier_id = supplierId; // 🔹 supplier-wise filter

      const res = await axios.get("/api/admin/issue-notes", {
        headers,
        params,
      });
      ...
      console.error("Error fetching admin issue notes:", err);
      if (err.response?.status === 401 || err.response?.status === 403) {
        safeLogout();
      } else {
        setMessage("❌ Failed to load issue notes (admin).");
      }
    } finally {
      setLoading(false);
    }
  ...
  const handlePrintList = async () => {
    // Preload items so list print tables are populated
    await Promise.all(
      (notes || []).map((n) =>
          ...
          {viewMode === "cards" && (
            <div className="grid 2xl:grid-cols-2 gap-6">
              {notes.map((n) => (
                <div
                  key={n.issue_id}
                  id={`admin-issue-card-${n.issue_id}`}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      ...
                      <div className="text-gray-700 text-lg">
                        ...
                        {n.item_count ??
                          (itemsByNote[n.issue_id]?.length || 0)}
                      </div>
                      {n.is_edited && (
                        <div className="text-xs text-amber-700 bg-amber-50 inline-flex px-2 py-1 rounded-lg border border-amber-200 mt-1">
                          Edited by {n.edited_by_username || "unknown"} on{" "}
                          {n.edited_at
      ...
      <div className="print-area">
        <div style={{ fontFamily: "Arial, sans-serif", fontSize: 11 }}>
          <div style={{ textAlign: "center", marginBottom: 8 }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, marginTop: 6 }}>Admin – Issue Notes</div>
          </div>
              ...
              <div key={`print-${n.issue_id}`} style={{ marginBottom: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
                  ...
                  <div style={{ textAlign: "right" }}>
                    ...
                    <div>Lorry: {n.lorry_no || n.lorry_name || "-"}</div>
                    <div>Created by: {n.creator_username || "-"}</div>
                    {showPerNotePrinted && (
                      <div>Printed: {new Date().toLocaleString()}</div>
                    )}
                  </div>
...
export default AdminIssueNotes;
