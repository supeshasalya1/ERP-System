// client/src/Pages/Admin/AdminExpiredStore.jsx
import React, { useEffect, useMemo, useState } from "react";
import axios from "axios";
import { useNavigate } from "react-router-dom";
import AdminNavbar from "./AdminNavbar";

const AdminExpiredStore = () => {
    ...
    (async () => {
      try {
        const [supRes, storeRes] = await Promise.all([
          ...
          // expire store is global, same endpoint ok for admin
        ...
        if (err.response?.status === 401 || err.response?.status === 403) {
          ...
          setMessage("❌ Failed to load expire store.");
        }
      }
    })();
  ...
  const fetchStore = async (opts = {}) => {
    ...
    try {
      ...
      if (err.response?.status === 401 || err.response?.status === 403) {
        ...
        setMessage("❌ Error fetching expire store.");
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-7xl mx-auto mt-8 px-4 md:px-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Read-only view of all expired stock currently in expire store
                (separate from main stock).
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <button
              type="button"
              onClick={() => navigate("/admin/expired/report")}
...
export default AdminExpiredStore;
